        ...
        <div class="rounded-3xl px-8 py-10 sm:px-10" style="background: rgba(255,255,255,0.10); backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); border: 1px solid rgba(255,255,255,0.20); box-shadow: 0 24px 64px rgba(0,0,0,0.35);">
            ...
            <div class="flex flex-col items-center mb-8">
                <a href="/" class="flex items-center gap-3 group mb-1">
                    ...
                    <span class="text-xl font-bold text-white tracking-tight">E-<span class="text-brand-400">Canteen</span></span>
                </a>
                <p class="text-sm text-white/50 mt-1">Pre-Order System · SMK Negeri 40 Jakarta</p>
            </div>

            <h1 class="font-serif text-2xl font-bold text-white text-center mb-6">Buat Akun Baru</h1>

            {{-- Error Alert --}}
            @if ($errors->any())
                <div class="mb-5 bg-red-500/20 border border-red-400/40 text-red-200 text-sm rounded-xl px-4 py-3 space-y-1">
            ...
            <a href="/auth/google/redirect"
               class="flex items-center justify-center gap-3 w-full rounded-xl py-3 px-4 text-sm font-semibold text-white hover:bg-white/20 transition-colors duration-200 mb-5"
               style="border: 1px solid rgba(255,255,255,0.30); background: rgba(255,255,255,0.10);">
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-4">
                ...
                {{-- Name --}}
                <div>
                    <label for="name" class="block text-xs font-semibold text-white/70 mb-1.5 uppercase tracking-wide">Nama Lengkap</label>
                    <input
                        type="text" id="name" name="name"
                        value="{{ old('name') }}"
                        required autocomplete="name"
                        placeholder="Nama kamu"
                        class="w-full px-4 py-3 text-sm text-white placeholder-white/30 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent transition @error('name') ring-2 ring-red-400 @enderror"
                ...
                {{-- Role --}}
                <div>
                    <label class="block text-xs font-semibold text-white/70 mb-1.5 uppercase tracking-wide">Daftar sebagai</label>
                    <div class="grid grid-cols-2 gap-3">
                        <label id="role-student" class="flex items-center gap-2.5 px-4 py-3 rounded-xl cursor-pointer transition-all duration-200
                            {{ old('role', 'student') === 'student' ? 'ring-2 ring-brand-400' : '' }}"
                            style="background: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.20);">
                            <input type="radio" name="role" value="student" class="text-brand-500 focus:ring-brand-400"
                                {{ old('role', 'student') === 'student' ? 'checked' : '' }}>
                            <span class="text-sm font-medium text-white">Siswa</span>
                        </label>
                        <label id="role-teacher" class="flex items-center gap-2.5 px-4 py-3 rounded-xl cursor-pointer transition-all duration-200
                            {{ old('role') === 'teacher' ? 'ring-2 ring-brand-400' : '' }}"
                            style="background: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.20);">
                            <input type="radio" name="role" value="teacher" class="text-brand-500 focus:ring-brand-400"
                                {{ old('role') === 'teacher' ? 'checked' : '' }}>
                            <span class="text-sm font-medium text-white">Guru</span>
                        </label>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input
                            ...
                            class="w-full px-4 py-3 text-sm text-white placeholder-white/30 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent transition pr-11 @error('password') ring-2 ring-red-400 @enderror"
                            style="background: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.20);"
                        >
                        <button type="button" onclick="togglePass('password','eye1-open','eye1-closed')" class="absolute right-3 top-1/2 -translate-y-1/2 text-white/40 hover:text-white/80">
                ...
                {{-- Confirm Password --}}
                <div>
                    <label for="password_confirmation" class="block text-xs font-semibold text-white/70 mb-1.5 uppercase tracking-wide">Konfirmasi Password</label>
                    <div class="relative">
                        <input
                            ...
                            style="background: rgba(255,255,255,0.10); border: 1px solid rgba(255,255,255,0.20);"
                        >
                        <button type="button" onclick="togglePass('password_confirmation','eye2-open','eye2-closed')" class="absolute right-3 top-1/2 -translate-y-1/2 text-white/40 hover:text-white/80">
            ...
            <p class="text-center text-sm text-white/60 mt-6">
                Sudah punya akun?
                <a href="{{ route('login') }}" class="font-semibold text-brand-300 hover:text-brand-400 transition-colors">Masuk</a>
            </p>
        ...
        document.querySelectorAll('input[name="role"]').forEach(function(radio) {
            radio.addEventListener('change', function() {
                document.querySelectorAll('input[name="role"]').forEach(function(r) {
                    r.closest('label').classList.remove('ring-2', 'ring-brand-400');
                });
                this.closest('label').classList.add('ring-2', 'ring-brand-400');
            });
        });
    </script>
